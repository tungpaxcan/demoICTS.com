...
namespace ICTS.com.Areas.ICTS.Controllers
{
    ...
    public class DownloadsController : BaseController
    {
        ...
        public string UploadDownload(HttpPostedFileBase file)
        {
            if (ModelState.IsValid)
            {
                if (file != null && file.ContentLength > 0)
                {
                    ...
                    _FileName = sub11 + sub21 + "Download"  + file.FileName;
        ...
        public JsonResult Download1()
        {
            try
            {
                var dmspp = (from s in db.Downloads.Where(x => x.Id > 0)
                             select new
                             {
                                 id = s.Id,
                                 name = s.Name,
        ...
        [HttpGet]
        public JsonResult Download(string seach, int page)
        {
            try
            {
                var pageSize = 5;
                var dmspp = (from s in db.Downloads.Where(x => x.Id > 0)
                             select new
                             {
                                 ...
                                 status = s.Status == true ? "Hiển thị" : "Không hiển thị"
                                            ...
                                            x.download.ToLower().Contains(seach)
                                           );
        ...
        [HttpPost]
        public JsonResult Add(string name, bool status, string download)
        {
            try
            {
                ...
                var d = new Download();
                var images = download.Substring(8);
                d.Download1= images;
                d.Name = name;
        ...
        [HttpPost]
        public JsonResult Edit(int id, string name,bool status, string download)
        {
            try
            {
                var images = download.Substring(8);
                var session = (Admin)Session["admin"];
                var nameAdmin = session.Name;
                var dmsp = db.Downloads.SingleOrDefault(x => x.Id == id);
                dmsp.Name = name;
                dmsp.Status = status;
                dmsp.Download1 = images;
        ...
        public JsonResult Detail(int id)
        {
            try
            {
                ...
                var tv = db.Downloads.SingleOrDefault(x => x.Id == id);
