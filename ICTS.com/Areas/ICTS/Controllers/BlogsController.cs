...
namespace ICTS.com.Areas.ICTS.Controllers
{
    ...
    public class BlogsController : BaseController
    {
        ...
        [HttpGet]
        public JsonResult Blog(string seach, int page)
        {
            try
            {
                var pageSize = 5;
                var dmspp = (from s in db.Blogs.Where(x => x.Id > 0)
                             select new
                             {
                                 id = s.Id,
                                 name = s.Name,
        ...
        public JsonResult Edit(int id, string name, string url, string title,bool status, bool statusimage, string image)
        {
            try
            {
                ...
                var session = (Admin)Session["admin"];
                var nameAdmin = session.Name;
                var dmsp = db.Blogs.SingleOrDefault(x => x.Id == id);
                dmsp.Name = name;
                dmsp.Meta = url;
                dmsp.Title = title;
                dmsp.Status = status;
        ...
        public JsonResult Detail(int id)
        {
            try
            {
                ...
                var tv = db.Blogs.SingleOrDefault(x => x.Id == id);
